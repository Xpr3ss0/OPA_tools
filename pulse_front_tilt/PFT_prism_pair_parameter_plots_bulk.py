...
from materials import n_BBO, n_CaF2, n_FS, n_SF11, n_SF10, n_YAG, n_Sapphire, n_Spinel, n_MgF2_o, n_MgF2_e, n_BK7, n_LiF, n_ALON
import numpy as np
...
theta_1_range_deg = (4, 60)                            # apex angle of first prism in degrees
lmd = 400                                               # wavelength in nm
bandwidth = 10                                         # bandwidth in nm
materials_list = [n_BBO, n_CaF2, n_FS, n_YAG, n_Sapphire, n_MgF2_o, n_LiF, n_Spinel, n_ALON]
...
# iterate over all combinations of materials (excluding duplicates)
for mat_i, n1_func in enumerate(materials_list):
    for n2_func in materials_list[mat_i + 1:]:
        # refractive indices
        ...
        # plot angles
        ax1.plot(np.degrees(theta_1_array), np.degrees(theta_2_array), label='Prism pair angle combination')
        ...
        ax1.legend()
        ax1.grid()
        mat_1_name = n1_func.__name__[2:]
        mat_2_name = n2_func.__name__[2:]
        fig.suptitle(f'Prism pair parameters for {mat_1_name} and {mat_2_name} at {lmd} nm', fontsize=14)
        ...
        if save_plots:
            plot_name = f"{mat_1_name}_{mat_2_name}"
            fig.savefig(f"{main_plot_savedir}/{plot_name}.png", dpi=300)
        ...
        # plot reflectance
        fig_reflect, ax_reflect = plt.subplots(figsize=(6, 4))
        phi_i1_array = np.arcsin(n1 * np.sin(theta_1_array /2)) # incident angle for symmetric configuration
        result_arrays = prism_pair_angles(phi_i1_array, theta_1_array, theta_2_array, n1, n2)
        R_1_s, R_1_p = fresnel_reflectance(1, n1, result_arrays['phi_i1'])
        R_2_s, R_2_p = fresnel_reflectance(n1, n2, theta_1_array - result_arrays['phi_r1'])
        R_3_s, R_3_p = fresnel_reflectance(n2, 1, result_arrays['phi_i2'])
        R_4_s, R_4_p = fresnel_reflectance(n2, 1, theta_2_array - result_arrays['phi_r2'])
        T_total_s = (1 - R_1_s) * (1 - R_2_s) * (1 - R_3_s) * (1 - R_4_s)
        T_total_p = (1 - R_1_p) * (1 - R_2_p) * (1 - R_3_p) * (1 - R_4_p)

        ax_reflect.plot(np.degrees(theta_1_array), T_total_s, label='Total transmission')
        ax_reflect.plot(np.degrees(theta_1_array), T_total_p, label='Total transmission (p-pol)')
        ax_reflect.set_xlabel('First prism apex angle (deg)')
        ax_reflect.set_ylabel('Transmission')
        ax_reflect.legend()
        ax_reflect.grid()
